...
classifier = SVC(kernel = 'sigmoid') #since two class problem, used sigmoid activation
classifier.fit(X_train_norm, y_train)

# predicting the test set result
y_pred_svm = classifier.predict(X_test_norm)

# saving the result into pandas df accoring to the desired format
result = pd.DataFrame({'ID': ID_test, 'Attrition': y_pred_svm})

# save the file in csv format
result.to_csv('result_onehot_SVM.csv', encoding='utf-8', index=False)